...
async function initApp() {
  ...
  if (state.db) {
    ...
    onAuthStateChanged(state.auth, (user) => {
      ...
      if (user) {
        const isSuperAdminEmail = user.email === 'subhashlodhi8463@gmail.com';
        
        if (isSuperAdminEmail && state.db) {
          // Attempt to register directly (works if we are subhashlodhi8463@gmail.com or already authorized)
          setDoc(doc(state.db, 'admins', user.uid), {
            email: user.email,
            createdAt: new Date().toISOString()
          })
          .then(() => {
            console.log("Admin registration confirmed for:", user.email);
          })
          .catch(err => {
            console.warn("Direct admin registration failed, creating fallback claim in visits:", err);
            // If direct write failed (e.g. subhash who is not in rules yet), write a claim into public 'visits' collection
            setDoc(doc(state.db, 'visits', 'claim_' + user.uid), {
              type: 'admin_claim',
              email: user.email,
              uid: user.uid,
              createdAt: new Date().toISOString()
            }).catch(claimErr => console.error("Failed to write admin claim:", claimErr));
          });
        }

        // If subhashlodhi8463@gmail.com logs in, automatically promote pending claims
        if (user.email === 'subhashlodhi8463@gmail.com' && state.db) {
          try {
            const claimsQuery = query(collection(state.db, 'visits'), where('type', '==', 'admin_claim'));
            getDocs(claimsQuery).then(snap => {
              snap.forEach(async (claimDoc) => {
                const claim = claimDoc.data();
                if (claim.uid && claim.email === 'subhashlodhi8463@gmail.com') {
                  console.log("Super Admin auto-promoting claim for:", claim.email, "UID:", claim.uid);
                  await setDoc(doc(state.db, 'admins', claim.uid), {
                    email: claim.email,
                    createdAt: new Date().toISOString()
                  });
                  await deleteDoc(doc(state.db, 'visits', claimDoc.id));
                  console.log("Claim successfully authorized and cleaned up!");
                }
              });
            }).catch(e => console.warn("Error processing claims:", e));
          } catch (claimsErr) {
            console.error("Claims error:", claimsErr);
          }
        }
  ...
  // Safety Timeout: If loading takes longer than 4.5 seconds under slow networks, remove the loader anyway
  setTimeout(() => {
    removeLoaderImmediate();
  }, 4500);
...
function checkFirstLoadComplete() {
  if (settingsLoaded && categoriesLoaded && servicesLoaded) {
    const loader = document.getElementById('smm-global-loader');
    if (loader) {
...
function syncSettings() {
  ...
  unsubscribeSettings = onSnapshot(settingsRef, (docSnap) => {
    if (docSnap.exists()) {
      ...
      if (typeof state.settings.minDeposit === 'undefined' || state.settings.minDeposit < 30) {
        state.settings.minDeposit = 30;
      }
    } else {
...
function syncCategories() {
  ...
  unsubscribeCategories = onSnapshot(catCol, (snap) => {
    ...
    state.categories = list.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
...
function syncServices() {
  ...
  unsubscribeServices = onSnapshot(srvCol, (snap) => {
    const serviceMap = {};
    DEFAULT_SERVICES.forEach(srv => {
      serviceMap[srv.id] = { ...srv };
    });

    snap.forEach(docSnap => {
      const data = docSnap.data();
      serviceMap[data.id] = data;
    });
    ...
    // Also update admin services view in real-time if it is currently open
    const srvSearch = document.getElementById('admin-srv-search');
    if (srvSearch) {
...
async function silentlySeedDefaultServices() {
  ...
  try {
    ...
    if (!srvSnap.empty) {
      console.log("Services already exist in Firestore. Skipping silent seeding to preserve custom edits.");
      return;
    }
...
function renderHomeView() {
  ...
  const rawMin = state.settings && typeof state.settings.minDeposit !== 'undefined' ? state.settings.minDeposit : 30;
  const minVal = parseFloat(rawMin) || 30;
  const homeDepositInput = document.getElementById('home-deposit-amount');
  if (homeDepositInput) {
    homeDepositInput.placeholder = `Min ₹${minVal} to Deposit...`;
    homeDepositInput.setAttribute('min', minVal);
  }

  // Dynamically render home deposit chips based on minDeposit so we don't have invalid select chips
  const chipContainer = document.getElementById('home-chips-container');
  if (chipContainer) {
    const rawChips = [30, 50, 80, 120];
    const chips = rawChips.map(c => c < minVal ? minVal : c);
    const uniqueChips = [...new Set(chips)].sort((a, b) => a - b).slice(0, 4);
    
    // Fill up to 4 elements if needed
    while (uniqueChips.length < 4) {
      const last = uniqueChips[uniqueChips.length - 1] || minVal;
      uniqueChips.push(last + Math.round(minVal * 0.5));
    }

    // Set default input value if currently empty to match the default active chip (index 2)
    if (homeDepositInput && !homeDepositInput.value && uniqueChips[2]) {
      homeDepositInput.value = uniqueChips[2];
    }

    chipContainer.innerHTML = uniqueChips.map((c, idx) => {
      // Keep third chip highlighted by default initially, or based on input value
      const isDefaultActive = homeDepositInput && parseFloat(homeDepositInput.value) === c || (!homeDepositInput?.value && idx === 2);
      const btnClass = isDefaultActive
        ? "py-1.5 border border-[#5B3DF5] rounded-xl bg-indigo-50/50 dark:bg-indigo-950/20 text-[10px] font-black text-[#5B3DF5] dark:text-indigo-400 cursor-pointer text-center transition active:scale-95"
        : "py-1.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 text-[10px] font-black text-slate-500 dark:text-slate-400 hover:border-[#5B3DF5] transition cursor-pointer text-center active:scale-95";
      
      return `<button id="home-chip-${c}" onclick="window.selectHomeDepositChip(${c})" class="${btnClass}">₹${c}</button>`;
    }).join('');
  }
...
window.selectHomeDepositChip = function(amount) {
  ...
  // Highlight active chip dynamically inside home-chips-container
  const container = document.getElementById('home-chips-container');
  if (container) {
    const buttons = container.querySelectorAll('button');
    buttons.forEach(btn => {
      const idStr = btn.id || '';
      const chipAmount = parseInt(idStr.replace('home-chip-', '')) || 0;
      if (chipAmount === amount) {
        btn.className = "py-1.5 border border-[#5B3DF5] rounded-xl bg-indigo-50/50 dark:bg-indigo-950/20 text-[10px] font-black text-[#5B3DF5] dark:text-indigo-400 cursor-pointer text-center transition active:scale-95";
      } else {
        btn.className = "py-1.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 text-[10px] font-black text-slate-500 dark:text-slate-400 hover:border-[#5B3DF5] transition cursor-pointer text-center active:scale-95";
      }
    });
  }
};
...
window.handleHomeAddFunds = function() {
  ...
  const val = rawVal === '' ? minVal : parseFloat(rawVal);
...
function calculateCharge() {
  ...
  const displayCharge = Math.round(charge);

  if (costDisplay) {
    costDisplay.textContent = `₹${displayCharge}`;
  }
  ...
  if (submitBtn) {
    if (displayCharge > 0) {
      submitBtn.textContent = `SUBMIT SMM ORDER - ₹${displayCharge}`;
    } else {
...
window.submitWizardOrder = async function() {
  if (window.submitWizardOrder.isSubmitting) {
    return;
  }
  window.submitWizardOrder.isSubmitting = true;

  const srvId = document.getElementById('order-service-select-value')?.value || state.selectedServiceId;
  const qtyInput = document.getElementById('order-quantity-input');
  const linkInput = document.getElementById('order-link-input');

  if (!srvId || !qtyInput || !linkInput) {
    window.submitWizardOrder.isSubmitting = false;
    return;
  }
  ...
  const charge = Math.round((qty / 1000) * srv.pricePer1000);
...
function renderAdminDashboard() {
  ...
  // Background claims processor for Super Admin to automatically promote subhashlodhi8463@gmail.com
  if (state.currentUser && state.currentUser.email === 'subhashlodhi8463@gmail.com' && state.db) {
    try {
      const claimsQuery = query(collection(state.db, 'visits'), where('type', '==', 'admin_claim'));
      getDocs(claimsQuery).then(snap => {
        snap.forEach(async (claimDoc) => {
          const claim = claimDoc.data();
          if (claim.uid && claim.email === 'subhashlodhi8463@gmail.com') {
            console.log("Dashboard background process promoting admin claim for:", claim.email);
            await setDoc(doc(state.db, 'admins', claim.uid), {
              email: claim.email,
              createdAt: new Date().toISOString()
            });
            await deleteDoc(doc(state.db, 'visits', claimDoc.id));
            console.log("Dashboard background process successfully promoted claim!");
          }
        });
      }).catch(e => console.warn("Background claims check failed:", e));
    } catch (e) {
      console.warn("Background claims processor error:", e);
    }
  }
...
window.openServiceEditor = function(srvId) {
  ...
  if (srv) {
    ...
    // Use actual fixed package price instead of Price/10K
    const packagePrice = Math.round(((srv.minQuantity || 0) * (srv.pricePer1000 || 0)) / 1000);
    document.getElementById('edit-srv-price').value = packagePrice;
...
window.saveServiceChanges = async function() {
  ...
  const packagePriceInput = parseFloat(document.getElementById('edit-srv-price').value) || 0;
  ...
  // Calculate pricePer1000 dynamically based on the packagePrice input and minQuantity
  const pricePer1000 = parseFloat(((packagePriceInput * 1000) / minQuantity).toFixed(6));
    ...
    <form id="admin-settings-form" class="space-y-4 bg-slate-900 border border-slate-800 rounded-2xl p-4 text-xs" onsubmit="window.saveGeneralSettings(event)">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div>
          ...
          <input type="number" id="set-min-deposit" value="${typeof state.settings.minDeposit !== 'undefined' && state.settings.minDeposit !== null ? state.settings.minDeposit : 30}" class="w-full px-2.5 py-2 bg-slate-950 border border-slate-850 text-emerald-400 font-extrabold rounded-lg outline-none">
        </div>
        <div>
      ...
      <div class="border-t border-slate-800 pt-3 space-y-2">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div class="bg-slate-950 border border-slate-850 p-2 rounded-lg space-y-1">
            ...
            <input type="number" id="set-bonus-1-amt" value="${state.settings.bonusOffer1Amt || 120}" class="w-full px-1.5 py-1 bg-slate-900 border border-slate-850 rounded text-[10px] text-white">
          ...
          <div class="bg-slate-950 border border-slate-850 p-2 rounded-lg space-y-1">
            ...
            <input type="number" id="set-bonus-3-amt" value="${state.settings.bonusOffer3Amt || 80}" class="w-full px-1.5 py-1 bg-slate-900 border border-slate-850 rounded text-[10px] text-white">
...
window.saveGeneralSettings = async function(e) {
  ...
  const minDepositVal = document.getElementById('set-min-deposit').value;
  let minDeposit = minDepositVal !== '' ? parseFloat(minDepositVal) : 30;
  if (isNaN(minDeposit) || minDeposit < 30) {
    alert("Warning: Minimum deposit cannot be less than ₹30. System has auto-clamped it to ₹30.");
    minDeposit = 30;
    const minDepEl = document.getElementById('set-min-deposit');
    if (minDepEl) minDepEl.value = 30;
  }
  ...
  const bonusOffer1Amt = parseFloat(document.getElementById('set-bonus-1-amt').value) || 120;
  ...
  const bonusOffer3Amt = parseFloat(document.getElementById('set-bonus-3-amt').value) || 80;
...
function renderAddMoneyView() {
  ...
  if (addMoneyInput) {
    ...
    // Pre-fill if currently empty so user has a valid default starting amount
    if (!addMoneyInput.value) {
      addMoneyInput.value = minVal;
    }
  }
  ...
  // Dynamically hide bonus offer chips if their deposit amount is less than minDeposit
  const chip1 = document.getElementById('bonus-chip-1');
  if (chip1) {
    if (offer1Amt < minVal) chip1.classList.add('hidden');
    else chip1.classList.remove('hidden');
  }
  const chip2 = document.getElementById('bonus-chip-2');
  if (chip2) {
    if (offer2Amt < minVal) chip2.classList.add('hidden');
    else chip2.classList.remove('hidden');
  }
  const chip3 = document.getElementById('bonus-chip-3');
  if (chip3) {
    if (offer3Amt < minVal) chip3.classList.add('hidden');
    else chip3.classList.remove('hidden');
  }
  const chip4 = document.getElementById('bonus-chip-4');
  if (chip4) {
    if (offer4Amt < minVal) chip4.classList.add('hidden');
    else chip4.classList.remove('hidden');
  }
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (succOverlay) {
            ...
            const offer1Amt = parseFloat(state.settings.bonusOffer1Amt) || 120;
            ...
            const offer3Amt = parseFloat(state.settings.bonusOffer3Amt) || 80;
...
window.confirmAndClaimDeposit = async function() {
  if (window.confirmAndClaimDeposit.isSubmitting) {
    return;
  }
  window.confirmAndClaimDeposit.isSubmitting = true;
  ...
  if (!/^\d{12}$/.test(utrVal)) {
    alert('Please enter a valid 12-digit numeric UPI UTR / Transaction ID.');
    window.confirmAndClaimDeposit.isSubmitting = false;
    return;
  }

  // Double-check duplicates in database / local
  if (state.db) {
    try {
      const q = query(collection(state.db, 'payment_requests'), where('utr', '==', utrVal));
      const qSnap = await getDocs(q);
      if (!qSnap.empty) {
        alert("⚠️ Duplicate Deposit Error!\n\nThis UTR / Transaction ID has already been submitted for verification. Please do not submit duplicate requests.");
        window.confirmAndClaimDeposit.isSubmitting = false;
        return;
      }
    } catch (e) {
      console.warn("Failed to complete remote duplicate UTR check:", e);
    }
  } else {
    const payments = JSON.parse(localStorage.getItem('local_payment_requests') || '[]');
    if (payments.some(p => p.utr === utrVal)) {
      alert("⚠️ Duplicate Deposit Error!\n\nThis UTR / Transaction ID has already been submitted locally.");
      window.confirmAndClaimDeposit.isSubmitting = false;
      return;
    }
  }
  ...
  try {
    ...
    if (err?.code === 'permission-denied' || err?.message?.includes('permission-denied') || err?.message?.includes('Permission denied')) {
      alert("⚠️ Firebase Database Permission Error!\n\nआपके Firebase Firestore Database के Rules लॉक हैं, जिसकी वजह से Deposit Request सबमिट नहीं हो पा रही है।\n\nकृपया अपने Firebase Console में जाकर Firestore Security Rules को 'firestore.rules' फ़ाइल के मुताबिक अपडेट करें।");
    } else {
      alert("Error submitting your deposit request. Please contact support via WhatsApp/Telegram.");
    }
    navigateTo('home');
  } finally {
    window.confirmAndClaimDeposit.isSubmitting = false;
  ...
  if (state.db) {
    const unsub = onSnapshot(doc(state.db, 'wallets', walletId), (snap) => {
      if (snap.exists()) {
        ...
        console.log(`[Wallet Info] Wallet document does not exist for ID ${walletId} in Firestore. Initializing...`);
        setDoc(doc(state.db, 'wallets', walletId), {
          balance: 0,
          id: walletId,
          updatedAt: new Date().toISOString()
        }).then(() => {
          console.log(`[Wallet Success] Successfully initialized 0-balance wallet for ID ${walletId}`);
        }).catch(err => {
          console.error(`[Wallet Error] Failed to initialize default 0-balance wallet for ID ${walletId}:`, err);
        });
      }
      renderWalletDisplays();
    }, (err) => {
...
function syncPayments() {
  if (state.db) {
    const walletId = state.currentUser ? state.currentUser.uid : state.guestId;
    const isAdminUser = !!state.currentUser;
    
    let q;
    if (isAdminUser) {
      q = collection(state.db, 'payment_requests');
    } else {
      q = query(collection(state.db, 'payment_requests'), where('userId', '==', walletId));
    }
...
window.approvePaymentRequest = async function(requestId) {
  console.log("=== STARTING PAYMENT APPROVAL DEBUG TRACE ===");
  console.log("Request ID to approve:", requestId);
  
  const req = state.paymentRequests.find(r => r.id === requestId);
  if (!req) {
    console.error("[Approval Error] Payment request not found in local state!");
    alert("Error: Payment request details not found. Please refresh the page and try again.");
    return;
  }

  console.log("Payment request payload found:", JSON.stringify(req));
  console.log("Firebase database status:", state.db ? "CONNECTED" : "OFFLINE/LOCAL");
  console.log("Current user logged in:", state.currentUser ? {
    uid: state.currentUser.uid,
    email: state.currentUser.email,
    emailVerified: state.currentUser.emailVerified
  } : "No user authenticated (GUEST mode)");

  // Robust validation of target userId
  const targetUserId = req.userId || req.uid;
  if (!targetUserId || targetUserId === 'undefined' || targetUserId === 'null') {
    console.error("[Approval Error] Invalid or missing userId in payment request!", req);
    alert(`Error: This payment request (${requestId}) is missing a valid User ID / Session ID. (Found: ${targetUserId}). Cannot credit balance to a non-existent wallet.`);
    return;
  }
  ...
  console.log("Calculated credited amount (Amount + Bonus):", creditedAmount, "based on base amount:", amt, "and bonus:", bonus);

  try {
    if (state.db) {
      const walletRef = doc(state.db, 'wallets', targetUserId);
      console.log("Attempting to read wallet for user UID:", targetUserId, "path: /wallets/" + targetUserId);
      ...
      try {
        ...
        if (walletSnap.exists()) {
          currentBal = parseFloat(walletSnap.data().balance) || 0;
          console.log("Wallet exists. Current user balance retrieved:", currentBal);
        } else {
          console.log("No wallet exists for this user. Starting balance will be 0.");
        }
      } catch (readErr) {
        console.error("Error reading user wallet document:", readErr);
        throw readErr;
      }

      console.log("Step 2: Building atomic WriteBatch with Firestore...");
      const batch = writeBatch(state.db);
      
      const prRef = doc(state.db, 'payment_requests', requestId);
      console.log("Adding update operation: Update /payment_requests/" + requestId + " to status 'Approved'");
      batch.update(prRef, { status: 'Approved' });
      
      const newBalanceVal = parseFloat((currentBal + creditedAmount).toFixed(2));
      console.log("Adding set operation: Set /wallets/" + targetUserId + " with balance:", newBalanceVal);
      batch.set(walletRef, { 
      ...
      console.log("Step 3: Attempting batch commit to Firestore...");
      await batch.commit();
      console.log("=== BATCH TRANSACTION COMMITTED SUCCESSFULLY ===");
    } else {
      console.log("No database connection. Simulating locally...");
    ...
    console.error("=== TRANSACTION FAIL DETECTED ===");
    console.error("Full error stack/details:", err);
    
    const errMsg = err?.message || err?.toString() || "Unknown Firestore Error";
    const isPermissionDenied = errMsg.includes("permission-denied") || errMsg.includes("Permission denied") || err?.code === 'permission-denied';
    
    if (isPermissionDenied) {
      console.log("Permission denied triggered. Displaying interactive resolution modal...");
      showRulesResolutionModal(errMsg, state.currentUser ? state.currentUser.email : "Not Logged In");
    } else {
...
window.rejectPaymentRequest = async function(requestId) {
  console.log("=== STARTING PAYMENT REJECTION DEBUG TRACE ===");
  console.log("Request ID to reject:", requestId);
  
  try {
    if (state.db) {
      console.log("Updating payment request status to 'Rejected' on Firestore...");
    ...
    console.error("Rejection error stack/details:", err);
    ...
    if (isPermissionDenied) {
      ...
      alert("Error rejecting payment: " + errMsg);
    }
  }
};

// Interactive overlay/modal to assist users with copying rules directly to solve permission errors
function showRulesResolutionModal(rawErrorMsg, userEmail) {
  // Remove existing if any
  const existing = document.getElementById('rules-resolution-modal');
...
service cloud.firestore {
  ...
  const modalHtml = `
    <div id="rules-resolution-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md">
      <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-xl w-full p-6 space-y-4 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
          <div class="flex items-center gap-2 text-rose-500">
            <span class="text-xl">⚠️</span>
            <h3 class="text-xs font-black uppercase tracking-wider text-white">Firestore Security Rules Setup Required</h3>
          </div>
          <button onclick="document.getElementById('rules-resolution-modal').remove()" class="text-slate-500 hover:text-white transition">
        ...
        <div class="text-[11px] text-slate-300 space-y-3 leading-relaxed">
          <p>आप <strong>${userEmail}</strong> से लॉग इन हैं, लेकिन आपके <strong>Firebase Project</strong> पर पुराने Security Rules डिप्लॉयड हैं, जिसकी वजह से भुगतान अप्रूव करने पर <span class="text-rose-400 font-bold">Permission Denied</span> आ रहा है।</p>
          
          <div class="bg-slate-950 p-3 rounded-lg border border-slate-850 space-y-1 font-mono text-[9px]">
            <div class="text-slate-400"><span class="text-slate-500">•</span> User: <span class="text-amber-400 font-bold">${userEmail}</span></div>
            <div class="text-slate-400"><span class="text-slate-500">•</span> UID: <span class="text-indigo-400">${state.currentUser ? state.currentUser.uid : 'N/A'}</span></div>
            <div class="text-rose-400"><span class="text-slate-500">•</span> Error: ${rawErrorMsg}</div>
          </div>

          <div class="space-y-1.5">
            <h4 class="text-xs font-extrabold text-white uppercase">इसे ठीक करने के 3 आसान तरीके (Solutions):</h4>
            <ol class="list-decimal list-inside space-y-1 text-slate-400 text-[10px]">
              <li>अपने <a href="https://console.firebase.google.com/" target="_blank" class="text-indigo-400 underline hover:text-indigo-300 font-bold">Firebase Console</a> में जाएँ।</li>
              <li>बाईं तरफ <strong>Build</strong> -> <strong>Firestore Database</strong> -> ऊपर <strong>Rules</strong> टैब पर क्लिक करें।</li>
              <li>नीचे दिए गए पूरे बॉक्स के कोड को कॉपी करें, वहाँ पुराने कोड को हटाकर इसे पेस्ट करें, और <strong>"Publish"</strong> पर क्लिक करें।</li>
            </ol>
          </div>

          <div class="relative">
            <textarea readonly id="diag-rules-box" class="w-full h-36 bg-slate-950 border border-slate-850 rounded-lg p-2.5 font-mono text-[9px] text-slate-400 focus:outline-none focus:border-indigo-500 select-all scrollbar-none">${rulesText}</textarea>
            <div class="absolute bottom-2 right-2">
              <button onclick="window.copyRulesTextFromModal()" class="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-[9px] font-black uppercase shadow-lg">
                Copy Code
              </button>
            </div>
          </div>
          
          <p class="text-[9px] text-slate-400 italic">रूल्स को अपडेट करने के तुरंत बाद वापस आकर 'Approve' बटन दबाएँ। सब कुछ तुरंत सफल होने लगेगा! ❤️</p>
        </div>

        <div class="flex gap-2 pt-1 border-t border-slate-800">
          <button onclick="window.copyRulesTextFromModal()" class="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-[10px] font-black uppercase tracking-wider">
            📋 Copy Rules & Close Instructions
          </button>
          <button onclick="document.getElementById('rules-resolution-modal').remove()" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[10px] font-black uppercase">
            Close
          </button>
        </div>
      </div>
...
window.copyRulesTextFromModal = function() {
  const box = document.getElementById('diag-rules-box');
  if (box) {
    const textToCopy = box.value;
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(textToCopy)
        .then(() => {
          alert("✅ Firestore Rules successfully copied to clipboard!\n\nअब इन्हें अपने Mobile/Desktop के Firebase Console -> Firestore -> Rules में पेस्ट करके 'Publish' दबाएँ।");
          const modal = document.getElementById('rules-resolution-modal');
          if (modal) modal.remove();
        })
        .catch(err => {
          console.warn("navigator.clipboard failed, trying fallback:", err);
          fallbackCopyText(box);
        });
    } else {
      fallbackCopyText(box);
    }
...
function fallbackCopyText(box) {
  ...
  try {
    ...
    if (successful) {
      ...
      alert("Failed to copy. Please manually select all code in the box and copy.");
    }
  } catch (e) {
    alert("Failed to copy. Please manually select all code in the box and copy.");
  }
